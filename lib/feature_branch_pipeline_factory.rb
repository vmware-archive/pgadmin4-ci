...
class FeatureBranchPipelineFactory
  ...
  def construct_pipeline_for_branches
    ...
    ['pgadmin', 'pgadmin36'].each do |pyenv_version|
        source_name = 'pivotal-'
        git_uri = 'https://github.com/greenplum-db/pgadmin4.git'
        @branches_list.each do |branch|
          add_branch_entry_to_section(@jobs_renderer, git_uri, source_name,branch, constructed_pipeline['jobs'], pyenv_version)
          add_branch_entry_to_section(@resources_renderer, git_uri, source_name, branch, constructed_pipeline['resources'], pyenv_version)
        end
  ...
  def add_branch_entry_to_section(renderer, git_uri, source_name, branch_name, pipeline_yaml_section, pyenv_version)
    ...
    renderer_binding.local_variable_set(:pyenv_version, pyenv_version)
